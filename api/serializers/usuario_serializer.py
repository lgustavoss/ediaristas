...
class UsuarioSerializer(serializers.ModelSerializer):
    ...
    # Validando a senha de um usuário
    ...
    def validate_nascimento(self, nascimento):
        ...
        if idade < 18:
            raise serializers.ValidationError("Usario menor de idade")
        if idade > 100:
            raise serializers.ValidationError("Idade maior que a permitida")
        return nascimento

    # Cadastrando uma diarista e definindo a média da reputação
    ...
    def create(self, validated_data):
        ...
        if validated_data['tipo_usuario'] == 2:
            reputacao_geral = Usuario.diarista_objects.reputacao_geral()[
                'reputacao__avg']
            if reputacao_geral is None:
                reputacao_geral = 5
        usuario = Usuario.objects.create(
            reputacao=reputacao_geral, **validated_data)
        return usuario
